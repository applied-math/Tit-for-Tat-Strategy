...
def Tit_For_Tat(
    p1, p2, i
):  # Player Cooperate in the first round. Then in each subsequent round, play the opponent's action in the previous round.
    ...
    if i == 0:
        ...
        p1[i] = p2[
            i - 1
        ]  # The other rounds is the opponent's action in the previous round.
    return p1[i]  # Return The Next Action of the player.


def Suspicious_TFT(
    p1, p2, i
):  # Player Defect in the first round. Then in each subsequent round, play the opponent's action in the previous round.
    # p1 : The Player 1  Actions.
    # p2 : The Player 2  Actions.
    if i == 0:
...
def Reverse_TFT(
    p1, p2, i
):  # Defect in the first round, then plays the reverse of the opponent's action in the previous round.
    # p1 : The Player 1  Actions.
    # p2 : The Player 2  Actions.
    if i == 0:
        p1[i] = 1  # Make the first round of the player Defect.
        ...
        p1[i] = (
            1 - p2[i - 1]
        )  # The other rounds is the Reverse of the opponent's action in the previous round.
    return p1[i]  # Return The Next Action of the player.
...
def Random(p, i):  # In each round, cooperate or defect with equal probabilities.
    ...
    p[i] = random.choice(
        actions
    )  # Make the action Cooperate or defect based on equal probabilities.
    return p[i]  # Return The Next Action of the player.


def Naive_Prober(
    p1, p2, i
):  # Cooperate in the first round. Then in each subsequent round, play the opponent's action in the previous round, but sometimes defect in lieu of cooperation with some probability.
    # p1 : The Player 1  Actions.
    # p2 : The Player 2  Actions.
    if i == 0:
        ...
        r = (
            random.random()
        )  # Random Number to check the probability of make the next action to be defect
        if 0 < r < 0.001:
            p1[i] = 1
        else:
